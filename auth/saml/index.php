...
define('SAML_INTERNAL', 1);
    ...
    global $CFG, $USER, $SAML_COURSE_INFO, $SESSION, $err, $db;
    ...
    if (!$valid_saml_session) {
        ...
        $saml_user_identify = $username;
       
        // store the hcrhsGroup extracted from Saml Attributes
        $hcrhsGroup = $saml_attributes['hcrhsGroup'][0];
        ...
        // Check if user exist
        $user_exists = get_record('user','username', $username);
        ...
        if ($USER === false) {
            $err['login'] = get_string("auth_saml_error_complete_user_login", "auth_saml", $username);
            saml_error($err['login'], '?logout', $pluginconfig->samllogfile);
        }

        $USER->hcrhsGroup=$hcrhsGroup;
        
        if (function_exists('saml_hook_post_user_created')) {
            saml_hook_post_user_created($USER);
        }
        
        unset($USER->hcrhsGroup);
