...
class yui_menu_plugin_outline extends yui_menu_plugin {
    ...
    function sections($config) {
        ...
        if (in_array($COURSE->format, array('weeks','weekscss'))) {
            $format = 'week';
            $marker = ceil((time()-$COURSE->startdate)/604800);
        } else {
            $format = 'topic';
        ...
        foreach ($sections as $section) {
            ...
            if (empty($text)) {
                ...
                $text = $this->truncate_html(filter_text($text,$COURSE->id),
                    $config);
            }
            ...
            if ($url === false) {
                $item = new yui_menu_item($this, $text, $CFG->pixpath . '/i/one.gif');
            } else {
                $item = new yui_menu_item_link($this, $text, $url, $CFG->pixpath . '/i/one.gif');
            }
            ...
            foreach ($sectionmods as $modnumber) {
                ...
                if (isset($modinfo->cms[$modnumber])) { // shortcut
                    if (!$modinfo->cms[$modnumber]->uservisible) continue;
                } else {
                ...
                $style .= $mod->visible ? "" : " dimmed ";
                $icon = "{$CFG->pixpath}/mod/{$mod->modname}/icon.gif";
