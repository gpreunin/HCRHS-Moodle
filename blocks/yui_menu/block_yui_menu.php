...
//]]>
    ...
    function script_children($children, $parent, $jsprefix) {
        ...
        foreach ($children as $k => $item) {
            ...
            $html = "<div class='icon column c0'>{$item->icon()}</div>" .
"<div class='column c1'>{$item->html()}</div>";
            $html = str_replace('"', '\"', $html);
            $script .= "
var $childname = new YAHOO.widget.HTMLNode(\"$html\", $parent, $expand);";
            if (isset($item->style)) {
...
class yui_menu_item_link extends yui_menu_item {
    ...
    function html() {
        ...
        // optional attributes: title and style
        $title = '';
        $style = '';
        if (isset($this->title)) {
        ...
        return "<a href='$url'$title $style>$txt</a>";
    }
}
