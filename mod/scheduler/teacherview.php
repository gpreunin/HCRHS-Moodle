...
if ($slots){
   ...
   if ($page == 'myappointments'){
    ...
    // prepare slots table - gwp 2010 Nov 19 - don't show Attendant (teacher/co-ordinator) column
    // can't figure how to eliminate scheduler name !!!
     $table->head  = array ('', $strdate, $strstart, $strend, $strstudents, $straction);
     $table->align = array ('CENTER', 'LEFT', 'LEFT', 'CENTER', 'CENTER', 'CENTER', 'LEFT', 'CENTER');
     $table->width = '80%'; 
    ...
    foreach($slots as $slot) {
        ...
        if ($slot->isappointed) {
            ...
            foreach($appointedstudents as $appstudent){
               ...
               // $name = "<a href=\"view.php?what=viewstudent&amp;id={$cm->id}&amp;studentid={$student->id}&amp;course={$scheduler->course}&amp;order=DESC\">".fullname($student).'</a>';
                $name = "<a href=\"view.php?what=viewstudent&amp;id={$cm->id}&amp;studentid={$student->id}&amp;course={$scheduler->course}&amp;order=DESC\">".'<span style="font-size: x-small;">'.fullname($student).'</span></a>';
       ...
       if ($slot->exclusivity > 1){
           ...
           if (($slot->exclusivity - $countstudentApp)==0){
             $actions .= '<br/><strong>FILLED</strong>';
           }
           else {
             $actions .= '<br/>('.($slot->exclusivity - $countstudentApp)."/".$slot->exclusivity.' openings)';
           }
        }
      /*  if ($slot->reuse){
...
if (!$students) {
   ...
   /* $mtable->head  = array ('', $strname, $stremail, $strseen, $straction);
    ...
    $mtable->align = array ('LEFT','LEFT','CENTER');
    $mtable->width = array('', '', '');
    ...
    foreach ($students as $student) {
        if (!scheduler_has_slot($student->id, $scheduler, true, $scheduler->schedulermode == 'onetime')) {
           ...
           // $checkbox .= '</a>';
            ...
            $mtable->data[] = array($name, $email, $actions);
        }
    }
