 ...
 */
    ...
    print_header($strtitle,$strheading,$navigation);
    ...
    if ($um->preprocess_files()) {
		...
		// Fill this with a list of comma separated id numbers to delete courses.
		$deleteids = "";
		$idnums = array();
		
		foreach($courseids as $courseid) {
                        
			if(!($mysqlresource = mysql_query('SELECT id,fullname, shortname FROM mdl_course WHERE id = '.$courseid.';')) ) {
				// Say we couldnt find that course
				notify('MySQL Querry Error on course with id: '.$courseid.'. Bulk Deleter will not delete this course.','notifyproblem');
				continue;
			}
			if(!mysql_num_rows($mysqlresource)) {
				...
				notify('Could not find course with id: '.$courseid.'. Bulk Deleter will not delete this course.','notifyproblem');
				continue;
			}
			...
			$fullname = $arr[1]; //second element should be fullname
                        $shortname = $arr[2]; // third element should be shortname
			
			if( isset($idnums[$id]) && $idnums[$id]) {
				notify('Duplicate entry found for course with id: '.$courseid.'... skipping','notifyproblem');
				continue;
			}
			else {
				$idnums[$id] = $fullname . " : " . $shortname;
		...
		echo '<form method="post" action="gwp_deletecourses.php">';
	...
	else if(isset($_POST['ids']) && $ids = $_POST['ids']) {
		...
		echo '<center>';
		echo '<form method="post" enctype="multipart/form-data" action="gwp_deletecourses.php">'.
		...
		echo '<font style="font-size:11pt;">';
		echo 'Please select an unformatted text file<br />The <i>id</i> of each course to be deleted should each be on a separate line<br /><br /><br />';
		echo 'For Example:<br /></font>';
		echo '<font style="font-family:monospace;font-size:8pt;">';
		echo '39<br />';
		echo '61<br />';
		echo '737<br /><br /><br /><hr /><br /><br />';
		echo '</font>';
